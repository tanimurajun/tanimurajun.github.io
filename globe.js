(function () {
     ...
     /* ---- normalize country id to 3-digit zero-padded string ---- */
     function normCountryId(v) {
            ...
            Promise.all([
                            ...
                            places.forEach(function (p) {
                                ...
                                .attr("fill", function (d) {
                                                        return visitedCountryCodes.has(normCountryId(d.id)) ? VISITED_COLOR : DEFAULT_COLOR;
                                })
                                .attr("stroke", BORDER_COLOR)
                                .attr("stroke-width", 0.4)
                                .on("mouseover", function (event, d) {
                                                        var key = normCountryId(d.id);
                                                        var name = countryNames[key] || d.properties.name || "ID:" + d.id;
                                                        var visited = visitedCountryCodes.has(key);
                                ...
                                .on("mouseout", function (event, d) {
                                                        d3.select(this).attr("fill", visitedCountryCodes.has(normCountryId(d.id)) ? VISITED_COLOR : DEFAULT_COLOR);
                            ...
                            gUS.selectAll("path")
                                ...
                                .on("mouseout", function (event, d) {
                                                        d3.select(this).attr("fill", visitedStateFIPS.has(d.id) ? VISITED_US_COLOR : DEFAULT_US_COLOR);
                                                        tooltip.style("display", "none");
                                });
